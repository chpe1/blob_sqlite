...
def search_column_blob(table):
    ...
    for element in table_info:
        if element["type"] == "BLOB" or element["type"] == "blob":
            ...
            print("*** The column " + element["name"] + " is a blob column ! ***")
...
def duplicates_colonnes_blob(table, dict_colonnes_blob):
    ...
    for colonne in dict_colonnes_blob.values():
        try:
            ...
            message = (
                message + "*** The column " + colonne + " has been created ! ***" + "\n"
            )
        except sqlite3.OperationalError:
            message = (
                message + "*** The column " + colonne + " already exists.***" + "\n"
            )
    return message
    ...
    for ligne in resultats_requete:
        ...
        if ligne[colonne_blob] is not None:
            for (string, type, span, is_interesting) in b2s.extract_all_strings(
                ligne[colonne_blob], only_interesting=True
            ):
                colonne_text = colonne_text + f"{string}" + "\n"
        if index == 1:
            print(
                "*** Work in progress in the column "
                + colonne_blob
                + " , please wait... ***"
            )
        cursor.execute(
            ...
            + colonne_blob
            + "= ?",
            (colonne_text, ligne[colonne_blob]),
        )
        connexion.commit()
    print(
        "*** Conversion of blob data to txt performed on the column : "
        + colonne_blob
        + ". ***"
    )
    ...
    if error == 0:
        ...
        for table in tables:
            ...
            if len(colonnes_blob) == 0:
                print(
                    "*** No blob column was found in the table "
                    + table["name"]
                    + " ***"
                )
